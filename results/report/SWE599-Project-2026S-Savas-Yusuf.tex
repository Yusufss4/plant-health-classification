...
\item \textbf{Runtime}: \textbf{ONNX Runtime}~\cite{onnxruntime} used from \textbf{C++}, not Python, so latency and FPS reflect the actual on-device stack instead of extra interpreter overhead.
...
\section{Testing, cross-compilation, and deployment}

Unit tests use \textbf{Catch2} (\path{cpp/tests/test_preprocess.cpp}) to validate tensor shape and channel handling, and \path{scripts/validate_cpp_inference.sh} checks Python-vs-C++ parity on identical preprocessed tensors. The Pi target is built with the cross toolchain in \path{cpp/toolchains/rpi-aarch64/}; \path{scripts/download_onnxruntime.sh} fetches the matching ONNX Runtime, and \path{scripts/deploy_rpi_zero2w.sh} pushes the binary and model to the device.
